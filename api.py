...
async def submit_answer(session_id: str, body: AnswerRequest):
    """Submit an answer to the current question. Returns the next question,
    or the compiled requirements when the last answer is submitted."""
    ...
    field_name = QUESTIONS[idx][0]
    session.answers[field_name] = body.answer.strip()
    ...
    # Last answer — compile requirements
    config = _load_config()
...
async def download_bot(folder_name: str):
    """Download a generated bot JSON file."""
    config = _load_config()
...
async def download_requirements(folder_name: str):
    """Download the requirements JSON for a generated bot."""
    config = _load_config()
    output_dir = Path(config.get("output_dir", "./output"))
...
async def list_outputs():
    """List all generated bot output folders."""
    config = _load_config()
    output_dir = Path(config.get("output_dir", "./output"))
